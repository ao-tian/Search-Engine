...
def test_tfidf_scores():
    """Test TF-IDF scoring on individual documents."""
    print("=" * 70)
    print("TF-IDF Word Importance Analysis")
    print("=" * 70)
    
    # Prepare documents_words for IDF calculation
    documents_words = []
    for doc_id in sorted(documents.keys()):
        words = get_words(documents[doc_id].lower())
        documents_words.append(words)
    
    # Show top words for a few documents
    test_docs = [0, 3, 5]  # Performance, CAPTCHA, Git documents
    
    for doc_id in test_docs:
        print(f"\nTop words in document {doc_id}:")
        print("-" * 70)
        words_list = documents_words[doc_id]
        unique_words = list(set(words_list))
        
        # Calculate TF-IDF for each word
        scores = {word: tfidf(word, words_list, documents_words) for word in unique_words}
        ...
        for word, score in sorted_words[:5]:
            tf_score = tf(word, words_list)
            idf_score = idf(word, documents_words)
            print(f"  Word: {word:15} | TF: {tf_score:.4f} | IDF: {idf_score:.4f} | TF-IDF: {score:.6f}")


def compare_baseline_vs_tfidf():
    """Compare baseline search vs TF-IDF enhanced search."""
    print("\n" + "=" * 70)
    print("Comparing Baseline vs TF-IDF Search")
    print("=" * 70)
    ...
    # Build both indices
    print("\nBuilding indices...")
    baseline_index = build_index(documents, v, use_tfidf=False)
    tfidf_index = build_index(documents, v, use_tfidf=True)
    print("Done!\n")
    ...
    for query in test_queries:
        ...
        # Baseline search
        baseline_results = search(query, baseline_index, documents, v, top_k=3, use_tfidf=False)
        
        # TF-IDF search
        tfidf_results = search(query, tfidf_index, documents, v, top_k=3, 
                              use_tfidf=True, documents_words=documents_words)
        
        print("\nBaseline Results:")
        for i, (score, preview, doc_id) in enumerate(baseline_results, 1):
            print(f"  {i}. [Score: {score:.6f}] Doc {doc_id}: {preview[:60]}...")
        
        print("\nTF-IDF Results:")
        for i, (score, preview, doc_id) in enumerate(tfidf_results, 1):
            print(f"  {i}. [Score: {score:.6f}] Doc {doc_id}: {preview[:60]}...")
        
        # Check if rankings differ
        baseline_doc_ids = [doc_id for _, _, doc_id in baseline_results]
        tfidf_doc_ids = [doc_id for _, _, doc_id in tfidf_results]
        
        if baseline_doc_ids != tfidf_doc_ids:
            print("\n  ⚠ Ranking differs between baseline and TF-IDF!")
        else:
            print("\n  ✓ Same ranking (but scores may differ)")


if __name__ == "__main__":
    test_tfidf_scores()
    compare_baseline_vs_tfidf()
    print("\n" + "=" * 70)
